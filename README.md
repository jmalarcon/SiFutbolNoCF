![Logo de SiFutbolNoCF](imgs/SiFutbolNoCF_small.webp)

**SiFutbolNoCF** es una herramienta para mitigar de forma inteligente **los bloqueos dinámicos** impuestos por proveedores de servicios de Internet (ISP) y **La Liga** a Cloudflare en España cada vez que hay fútbol emitido en plataformas de pago.
...
- 🗄️ **Caché temporal y resiliente de IPs de Cloudflare**: persiste en disco las IPs únicamente durante incidentes de bloqueo para tolerar reinicios del proceso, y se destruye automáticamente al reactivar el _proxy_.
...
Antes de arrancarlo hay que establecer los parámetros de funcionamiento: token de la API de Cloudflare (para poder poner y quitar el proxy), nombres de los dominios a controlar y sus zonas, etc...
...
El archivo `appsettings.local.json` es solo para desarrollo. Normalmente utilizarás `appsettings.json` o, si quieres ejecutarlo en un servidor en la nube o en un contenedor Docker, puede que te interese utilizar las variables de entorno.

### Opciones de configuración globales:

El programa dispone de las siguientes opciones globales establecidas bien en el archivo de configuración o como variables de entorno:

| Opción                            | Tipo     | Descripción                                                  | Variable de Entorno  | Valor por defecto                                  |
| --------------------------------- | -------- | ------------------------------------------------------------ | -------------------- | -------------------------------------------------- |
| `CfApiToken`                      | String   | Token de autenticación de Cloudflare (tipo portador / *Bearer*). | `CF_API_TOKEN`       | *(Requerido. Se explica más abajo cómo obtenerlo)* |
...
>[!IMPORTANT]
>Si en CloudEflare tienes configurados tanto el dominio principal (`midominio.com`) como el subdominio (`www.midominio.com`), debes **añadir ambos** a la monitorización si quieres que se desbloqueen adecuadamente.
...
> [!IMPORTANT]
> Si tu servidor web **no** tiene un certificado HTTPS propio asociado al dominio y solamente utiliza el de Cloudflare (usas Cloudflare [en modo "Flexible"](https://developers.cloudflare.com/ssl/origin-configuration/ssl-modes/flexible/), sin certificado en tu servidor) , **esto no te servirá** porque los navegadores modernos bloquean el acceso a sitios sin HTTPS válido. En este caso, configura un certificado SSL gratuito de Let's Encrypt o similar en tu servidor para que los usuarios puedan acceder sin problemas cuando el _proxy_ esté desactivado.
...
## Cómo obtener el token de Cloudflare

Puedes utilizar dos tipos de tókenes en Cloudflare: uno más sencillo y uno avanzado.

### 1. Obtener token de API de tipo Perfil (Específico de Zona) en Cloudflare
...
> [!IMPORTANT]
> Dado que este tipo de token no tiene privilegios de cuenta globales, no puede consultar la API de zonas para auto-descubrir IDs. **Implica obligatoriamente configurar el parámetro `CfZoneId`** para cada dominio en el archivo de configuración. El Id de zona lo puede ver en la portada del dominio en Cloudflare, en la zona de la derecha, muy abajo. No tiene pérdida.
...
## Configurar Notificaciones por Telegram (Opcional)
...
> [!IMPORTANT]
> El bot no puede enviarte mensajes si antes no has pulsado *Iniciar* con él.
...
```bash
./SiFutbolNoCF >> ./sifutbolnocf.log 2> /dev/tty &
```

> [!NOTE]
> El operador `>>` añade los logs al final del archivo, `2> /dev/tty` envía errores a la terminal, y el `&` final envía el proceso a segundo plano. Te devuelve el PID del proceso para matarlo más tarde con `kill <PID>`. Si cierras la terminal, el sistema finaliza el proceso. Si deseas mantener el proceso tras cerrar la terminal, usa `nohup ./SiFutbolNoCF >> /var/log/sifutbolnocf.log 2>&1 &`.
...
```powershell
Start-Process -FilePath ".\SiFutbolNoCF.exe" -RedirectStandardOutput "sifutbolnocf.log" -RedirectStandardError "sifutbolnocf.err.log"
```

> [!NOTE]
> - **Cerrar la terminal**: `Start-Process` crea un proceso independiente. Si cierras la ventana de Terminal o PowerShell, el programa **sigue ejecutándose en segundo plano**.
> - **Comprobar y detener**: puedes consultar si está activo con `Get-Process SiFutbolNoCF` y detenerlo con `Stop-Process -Name SiFutbolNoCF`.
> - **Modo oculto**: añade el parámetro `-WindowStyle Hidden` para no abrir pestañas en Windows Terminal.
>   - *Ventaja*: El proceso no muestra ninguna ventana en pantalla.
>   - *Inconveniente*: Debes recordar que sigue en ejecución y detenerlo a mano con `Stop-Process` cuando quieras pararlo (pero si lo pones en modo "daemon" no querrás detenerlo nunca. Puedes preparar una tarea programada para que se ejecute solo ante cualquier reinicio del sistema).

![Aspecto del terminal en Windows con los comandos explicados](imgs/windows-terminal.png)
